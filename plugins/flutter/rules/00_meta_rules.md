...
1. **If the user chooses Central Registry**:
   - Location: `~/Developer/agent-registry/plugins/flutter/rules/` (or the configured central registry path).
   ...
   - Update the rule and provide the Git command to commit and push the registry:
     `git -C <registry-path> commit -am "feat(flutter): update <rule-topic>"`
